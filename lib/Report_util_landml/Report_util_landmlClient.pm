...
<pre>
$params is a Report_util_landml.FilterContigsParams
$output is a Report_util_landml.FilterContigsResults
...
FilterContigsResults is a reference to a hash where the following keys are defined:
	...
	assembly_output has a value which is a Report_util_landml.assembly_ref
...
# Authentication: required
    ...
    my $result = $self->{client}->call($url, $self->{headers}, {
	    method => "Report_util_landml.filter_contigs",
	    params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
