package Report_util_landml::Report_util_landmlClient;
...
Report_util_landml::Report_util_landmlClient
...
sub new
{
    ...
    my $self = {
	client => Report_util_landml::Report_util_landmlClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
# Authentication: required
    ...
    my $result = $self->{client}->call($url, $self->{headers}, {
	    method => "Report_util_landml.assembly_metadata_report",
	    params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
# Authentication: required
    ...
    {
	Bio::KBase::Exceptions::ArgumentValidationError->throw(error =>
							       "Invalid argument count for function domain_report (received $n, expecting 1)");
    }
    {
	my($params) = @args;
	...
	my @_bad_arguments;
        (ref($params) eq 'HASH') or push(@_bad_arguments, "Invalid type for argument 1 \"params\" (value was \"$params\")");
        if (@_bad_arguments) {
	    my $msg = "Invalid arguments passed to domain_report:\n" . join("", map { "\t$_\n" } @_bad_arguments);
	    Bio::KBase::Exceptions::ArgumentValidationError->throw(error => $msg,
    ...
    my $url = $self->{url};
    my $result = $self->{client}->call($url, $self->{headers}, {
	    method => "Report_util_landml.domain_report",
	    params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
        ...
        Bio::KBase::Exceptions::HTTP->throw(error => "Error invoking method domain_report",
					    status_line => $self->{client}->status_line,
					    method_name => 'domain_report',
				       );
    }
...
sub status
{
    ...
    my $url = $self->{url};
    my $result = $self->{client}->call($url, $self->{headers}, {
        method => "Report_util_landml.status",
        params => \@args,
    });
    if ($result) {
        if ($result->is_error) {
...
sub version {
    ...
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "Report_util_landml.version",
        params => [],
    });
    if ($result) {
        if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for Report_util_landml::Report_util_landmlClient\n";
    }
    if ($sMajor == 0) {
        warn "Report_util_landml::Report_util_landmlClient version is $svr_version. API subject to change.\n";
    }
}
...
package Report_util_landml::Report_util_landmlClient::RpcClient;
